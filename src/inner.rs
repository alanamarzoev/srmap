        ...
        M: Clone,
    {
        ...
        pub fn insert(&mut self, k: K, v: Vec<V>, uid: usize) -> bool {
            ...
            if uid == 0 as usize {
                        ...
                        for v in vs {
                            if *v == *val && count >= last_seen && found == false {
                                self.b_map_r.get_and(&(k.clone(), val.clone()), |s| {
                                    ...
                                    for elem in s {
                                        match get_access(&elem[count].borrow(), uid) {
                                            true => {
                                                last_seen = count;
                                            ...
                                            false => {
                                                found = true;
                                                {
                                                    let mut foo = elem[count].borrow_mut();
                                                    update_access(&mut (*foo), uid, true);
                                                }

                                                let bmkey = (k.clone(), val.clone());
                                                // b_map_w.update(bmkey.clone(), elem);
                                                res = true;
                                            }
                                        }
                                    }
                ...
                if res {
                    // println!("updated bitmap.");
                } else {
                    // println!("going to add to umap.");
                }
